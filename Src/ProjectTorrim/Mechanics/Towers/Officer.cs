...
// Type: BrawlerSource.Mechanics.Towers.Officer
...
{
  public class Officer : Tower
  {
    public Officer(Layer layer, Position position)
      : base(layer, position, 200f)
    {
    }

    public Officer(GameObject parent, Position position)
    ...
    protected override void Construct(Position position, float diameter)
    {
      ...
      this.Cost = 15900;
      this.Description = "Fires a blunt projectile";
      this.Name = nameof (Officer);
      this.ProjectileInfo.Damage = 75;
      this.ProjectileInfo.MaxHitCount = 1;
      this.ProjectileInfo.Knockback = 2;
      this.ProjectileInfo.DamageType = DamageType.Blunt;
